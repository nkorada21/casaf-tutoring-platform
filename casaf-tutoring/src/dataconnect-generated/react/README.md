...
## GetTutorProfile
You can execute the `GetTutorProfile` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetTutorProfile(dc: DataConnect, vars: GetTutorProfileVariables, options?: useDataConnectQueryOptions<GetTutorProfileData>): UseDataConnectQueryResult<GetTutorProfileData, GetTutorProfileVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetTutorProfile(vars: GetTutorProfileVariables, options?: useDataConnectQueryOptions<GetTutorProfileData>): UseDataConnectQueryResult<GetTutorProfileData, GetTutorProfileVariables>;
```
...
```
### Return Type
Recall that calling the `GetTutorProfile` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetTutorProfile` Query is of type `GetTutorProfileData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetTutorProfile`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetTutorProfileVariables } from '@dataconnect/generated';
import { useGetTutorProfile } from '@dataconnect/generated/react'

export default function GetTutorProfileComponent() {
  // The `useGetTutorProfile` Query hook requires an argument of type `GetTutorProfileVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetTutorProfile(getTutorProfileVars);
  // Variables can be defined inline as well.
  const query = useGetTutorProfile({ id: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetTutorProfile(dataConnect, getTutorProfileVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetTutorProfile(getTutorProfileVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetTutorProfile(dataConnect, getTutorProfileVars, options);
  ...
  // If the Query is successful, you can access the data returned using the `UseQueryResult.data` field.
  if (query.isSuccess) {
    console.log(query.data.tutorProfile);
  }
  return <div>Query execution {query.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
...
export default function CreateReviewComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ sessionId: ..., studentId: ..., tutorProfileId: ..., rating: ..., comment: ..., });
